...
//İşte bu takip mekanizmasına Change Tracker denir. Change Traker ile nesneler üzerindeki değişiklikler/işlemler
//takip edilerek netice itibariyle bu işlemlerin fıtratına uygun sql sorgucukları generate edilir.
...
//urunler[6].UrunAdi = "Değişti 7"; // Update
...
//algıulanmasını opsiyonel olarak gerçekleştirmek isteyebiliriz. İşte bunun için
//DetectChanges fonksiyonu kullanılabilir ve her ne kadar EF Core değişikleri otomatik algılıyor
//olsa da siz yine de iradenizle kontrole zorlayabilirsiniz.
...
//İlgili metotlar(SaveChanges, Entries) tarafından DetectChanges metodunun
//otomatik olarak tetiklenmesinin konfigürasyonunu yapmamızı sağlayan proeportydir.
//SaveChanges fonksiyonu tetiklendiğinde DetectChanges metodunu içerisinde default olarak çağırmaktadır.
//Bu durumda DetectChanges fonksiyonunun kullanımını irademizle yönetmek ve maliyet/performans
//optimizasyonu yapmak istediğimiz durumlarda AutoDetectChangesEnabled özelliğini kapatabiliriz.
#endregion

#region Entries Metodu
//Context'te ki Entry metodunun koleksiyonel versiyonudur.
//Change Tracker mekanizması tarafından izlenen her entity nesnesinin bigisini
...
//Buradaki maliyetten kaçınmak için AuthoDetectChangesEnabled özelliğine false değeri verilebilir.

//var urunler = await context.Urunler.ToListAsync();

//urunler.FirstOrDefault(u => u.Id == 7).Fiyat = 123; //Update
//context.Urunler.Remove(urunler.FirstOrDefault(u => u.Id == 1001)); //Delete
...
//SaveChanges() veya SaveChanges(true) tetiklendiğinde EF Core herşeyin yolunda olduğunu varsayarak
//track ettiği verilerin takibini keser yeni değişikliklerin takip edilmesini bekler.
//Böyle bir durumda beklenmeyen bir durum/olası bir hata söz konusu olursa eğer
...
//ancak gerektiğinde yeniden oynatılabilmesi için değişikleri beklemeye/nesneleri takip etmeye devam eder.
...
//Urun urun = new() { Fiyat = 123, UrunAdi = "Ürün 1002" };
//Console.WriteLine(context.Entry(urun).State);
...
#region Modified
//Nesne üzerinde değşiiklik/güncelleme yapıldığını ifade eder.
...
public class ETicaretContext : DbContext
{
    ...
    {
        //İntercepter kullanılması
        var entries = ChangeTracker.Entries();
        foreach (var entry in entries)
        {
            if(entry.State == EntityState.Added)
            {
                Console.WriteLine("eklenen var");
            }
        }
