...
//Change Tracker, takip ettiği nesnelerin sayısıyla doğru orantılı olacak şekilde bir maliyete sahiptir.
//O yüzden üzerinde işlem yapılmayacak verilerin takip edilmesi bizlere lüzumsuz yere bir maliyet ortaya çıkaracaktır.

//AsNoTracking metodu, context üzerinden sorgu neticesinde gelecek olan verilerin
//Change Tracker tarafından takip edilmesini engeller.
...
//bu verileri istenilen noktalarda kullanabilir lakin veriler üzerinde herhangi bir değişiklik/update işlemi yapamayız.
...
//var kitaplar = await context.Kitaplar.Include(y => y.Yazarlar).AsNoTrackingWithIdentityResolution().ToArrayAsync();
...
public class Yazar
{
    public Yazar() => Console.WriteLine("Yazar Nesnesi Oluşutuldu.");
    public int Id { get; set; }
